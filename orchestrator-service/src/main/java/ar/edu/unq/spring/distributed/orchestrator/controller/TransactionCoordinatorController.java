...
public class TransactionCoordinatorController {
    ...
    public Publicacion onPublicar(@PathVariable("vendedorId") Long vendedorId,
                                  @PathVariable("itemId") Long itemId,
                                  @PathVariable("precio") BigDecimal precio) throws JsonProcessingException {
        return transactionalService.onPublicar(vendedorId, itemId, precio);
    }

    @PutMapping("/comprar/{compradorId}/publicacion/{publicacionId}")
    public Long onComprar(@PathVariable("compradorId") Long compradorId,
                          @PathVariable("publicacionId") Long publicacionId) {
        return transactionalService.onComprar(compradorId, publicacionId);
    }
